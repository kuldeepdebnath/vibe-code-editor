...
import { TemplateFolder } from '@/modules/playgrounds/lib/path-to-json';
    ...
    async function initializeWebContainer() {
      try {
        if (!sharedWebContainerBootPromise) {
          sharedWebContainerBootPromise = WebContainer.boot().then((webcontainerInstance) => {
            sharedWebContainer = webcontainerInstance;
            return webcontainerInstance;
          });
        }

        const webcontainerInstance = sharedWebContainer ?? await sharedWebContainerBootPromise;
